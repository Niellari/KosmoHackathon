...
**Статус: реализовано, но временно выключено по умолчанию.** Настраивается через
`training.gap_masking`; режим `test_like_blocks` создаёт 5 реплик с долей
скрытых точек 15% и распределением блоков длиной 1–5 как в test. После результата
0,0779 на платформе default возвращён на `leave_one_out`.

Ранее использовавшийся leave-one-out подход в основном скрывал отдельные
известные точки. Новый генератор приближает обучение к структуре test.
...
**Статус: реализовано.** Группы признаков независимо переключаются в
`features.temporal_dynamics`; расчёт общий для обучения и инференса.

Модель теперь получает следующие характеристики локальной формы кривой.
...
**Статус: реализовано и проверено.** `pchip_prediction` включён по умолчанию:
на фиксированной маске из 1000 точек RMSE изменился с 0,072058607 до
0,072020307. `local_quadratic`, разности и признаки согласия реализованы, но
выключены: в совместных абляциях они дали 0,072399925 и 0,072630570.
...
## 5. Расширенные исторические признаки

**Статус: реализовано и проверено, но выключено по умолчанию.** В конфиге
доступен единый расчёт `calculation: leave_one_season_out` и расширение
`expanded_statistics: true`. На маске из 3000 точек текущая готовая
климатология показала RMSE 0,070350394, расширенная пересчитанная история —
0,070765663. Поэтому production остаётся на `calculation: provided` и
`expanded_statistics: false`.

На трёх масках по 1000 точек результат был неоднозначным: средний RMSE улучшился
с 0,067108582 до 0,066966674, но более крупная проверка не подтвердила эффект.

Историю полигона можно описывать не только средним и стандартным отклонением.

Реализованные экспериментальные признаки:

```text
historical_mean_w{7,14,21}
historical_median_w{7,14,21}
historical_std_w{7,14,21}
historical_q25_w{7,14,21}
historical_q75_w{7,14,21}
historical_iqr_w{7,14,21}
historical_years_w{7,14,21}
historical_recent_weighted
historical_year_trend
```

Реализованы исторические окна `doy ± 7`, `± 14` и `± 21` день. В
`historical_recent_weighted` больший вес получают сезоны, расположенные ближе к
целевому году; скорость убывания задаётся через `recent_year_scale`.

## 6. Статистика других полей на ту же дату
...
## 7. Источник соседнего primary_ndvi
...
| V5 | Расширенные исторические признаки |

...
- До строки `platform test refresh` используется legacy test: 78 AOI и 3 112
  целей. После неё — актуальный test: 20 новых относительно train AOI и 2 323
  цели. Метрики через эту границу напрямую не сравниваются.
- `strict holdout` означает одинаковые целевые строки для пары моделей,
  одновременное скрытие признаков цели и отдельный контроль unseen-полигонов.
- Если указан один seed, результат считается поисковым; финальный вывод требует
  повторения либо подтверждения платформой.
- `proxy` обозначает расчётную оценку смеси, а не ответ hidden test.
- Критерий принятия: снижение RMSE при том же протоколе. Нулевая или
  нестабильная разница фиксируется как отклонённая гипотеза.

Главный актуальный результат: `0.1024 → 0.0736`, то есть снижение RMSE baseline
на **28.1%**. Строки журнала расположены хронологически, включая неудачные
опыты, чтобы финальный выбор можно было проследить.
...
## Абляция внешних данных на legacy test

Эти отправки проверяли не только факт добавления данных, но и чувствительность
к их географии и весу. Базовая точка для серии — `0.0763`; ниже лучше.

| обучающие внешние источники | веса | RMSE платформы | вывод |
|---|---:|---:|---|
| Зерноград, несколько сезонов | 0.10 | 0.0763 | эффекта нет |
| Зерноград, несколько сезонов | 0.50 | 0.0758 | небольшой устойчивый плюс |
| Зерноград, несколько сезонов | 0.60 | 0.0758 | плато |
| Зерноград, несколько сезонов | 0.75 | **0.0757** | лучший одиночный вес |
| Зерноград, несколько сезонов | 1.00 | 0.0758 | слишком большой вес не помогает |
| Егорлыкская без Зернограда | 0.10 / 0.25 / 0.50 | 0.0762 / 0.0760 / 0.0761 | источник слабее Зернограда, немонотонный эффект |
| Зерноград + Егорлыкская | 0.50 + 0.10 / 0.25 / 0.50 | 0.0759 / 0.0759 / 0.0758 | добавление второй зоны почти не меняет результат |
| смесь готовых прогнозов ZGD/EGL | 90/10, 75/25, 50/50 | **0.0757 / 0.0757 / 0.0757** | платформа почти нечувствительна к смеси |

Интерпретация: прирост не масштабируется с количеством внешних строк и зависит
от пространственного совпадения. Поэтому внешний корпус оставлен отдельными
именованными источниками, а production-флаг выключен. Это отрицательный, но
полезный результат: он остановил бесконтрольное увеличение датасета и привёл к
поиску ERA5-похожих зон и multi-sensor сбору.

## Проверяемость журнала

- параметры production-модели: `config.yaml`;
- код строгих масок: `src/synthetic.py`;
- код benchmark: `src/benchmark.py`;
- ранние платформенные ответы, имя файла и SHA-256:
  `artifacts/submissions/history.jsonl`;
- команды воспроизведения и расширенное объяснение механизмов: `REPORT.md`.

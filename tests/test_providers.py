...
from src.providers import GeeProvider, ProviderSettings, load_features, merge_primary
...
class FetchManyTest(unittest.TestCase):
    def test_fetch_many_combines_every_feature(self):
        source = {
            "type": "FeatureCollection",
            "features": [
                {
                    "type": "Feature",
                    "properties": {"id": "A", "crop_type": "зерновые"},
                    "geometry": SQUARE,
                },
                {
                    "type": "Feature",
                    "properties": {"id": "B", "crop_type": "подсолнечник"},
                    "geometry": SQUARE,
                },
            ],
        }
        ...
        class FakeProvider:
            def fetch(self, geometry, start, end, polygon_id, crop_type):
                return pd.DataFrame(
                    {
        ...
        result = GeeProvider.fetch_many(FakeProvider(), source, "2024-04-01", "2024-04-30")

        self.assertEqual(result["anon_polygon_id"].tolist(), ["A", "B"])
        self.assertEqual(result["crop_type"].tolist(), ["зерновые", "подсолнечник"])

    def test_service_account_key_is_optional(self):
        settings = ProviderSettings(project="example-project")

        self.assertIsNone(settings.key_path)
